    		...
    		lineModel.query().filter([['licenscee_id','=',ids[i]]]).all().done(function(item){
    			...
    			var myview = current.find('tr[data-id='+ item[0].licenscee_id[0] +']');
           		$.each(item,function(key, value){
						var d = new Date();
						var n = d.format('Y-m-d');
					        if (n>value.support_start) {
						myview.after("<tr class='oe_group_header'  id='" + value.id+ "' name='" + value.licenscee_id[0] + "' style='background:#fff;color:red'><td class='oe_list_group_name'><span style='float: left; white-space: pre;'></span><span class='ui-icon' style='float: left; background-position: 150px 150px'></span>" +
