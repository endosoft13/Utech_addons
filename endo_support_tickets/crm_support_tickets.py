...
class crm_support_tickets(osv.osv):
	...
	print "now i am in create method"
        pri_id = vals.get('priority',False)
	print "pri_id : ",pri_id
        x_cron_id=False
        if pri_id:
	    print "inside if pri_id :",pri_id
...
#         faq_obj= self.pool.get('crm_support_faq')
    ...
    _columns = {
       ...
       #Endosoft
        ...
        'user_id': fields.many2one('res.users', 'Responsible', required=True, track_visibility='onchange',),
...
#                     super(crm_support_tickets, self).write(cr, uid, ids, {'x_cron_id':x_cron_id,}, context)
	...
	print "Now we are in send_ack_email function"
        email_template_obj = self.pool.get('email.template')
        context = context is not None and context or {}

        ir_model_data = self.pool.get('ir.model.data')
	print "ir_model_data id : ",ir_model_data
        try:
	    print "I am in try block"
            template_id = ir_model_data.get_object_reference(cr, uid, 'endo_support_tickets', 'email_template_support_acknowledge_mail')[1]
	    print "print template id ",template_id
        except ValueError:
	    print "try block is False"
            template_id = False
	    print "template_id False value : ",template_id
     ...
     #               print "values::  ", values 
		    ...
		    print "values['subject'] : ",values['subject']
...
#                    values['body'] = body_html_part
   ...
   #                     self.write(cr, uid, ids, {'acknowledged':True,},context=None)
    ...
    def send_response_email(self, cr, uid, ids, context=None):
        ...
        try:
            template_id = ir_model_data.get_object_reference(cr, uid, 'endo_support_tickets', 'email_template_support_closed_mail')[1]
        except ValueError:
            template_id = False
...
#            ackd = tick.x_acknowledged
    ...
    def send_alert_email(self, cr, uid, ids, context=None):
        ...
        ir_model_data = self.pool.get('ir.model.data')
        try:
            template_id = ir_model_data.get_object_reference(cr, uid, 'endo_support_tickets', 'email_template_support_ticket_alert_mail')[1]
        except ValueError:
            template_id = False
        if template_id:
